...
const db = createClient({
  url: 'libsql://saadat-industries-prajapati107.aws-ap-south-1.turso.io',
  authToken: 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJpYXQiOjE3Nzk4OTc2NzgsImlkIjoiMDE5ZTZhMmItMTgwMS03ZDQ0LWFiYjQtYjEyMTU2NDVjMDhmIiwicmlkIjoiZDQ4NDExZWQtYjJlNi00OGMyLWIwNzgtMjEwMDVmNzI1YzAyIn0.742xkok75xfjbe7Jrd5aR7KYPmrbe1D4CqUiLbroShJZI_P7tCT4OURKMWel1rHvfnkn3D-sWqRpMIpKfvmmCQ',
});
...
await db.execute({
  sql: 'INSERT OR IGNORE INTO User (id, email, password, name, role, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
  args: [cuid(), 'admin@saadatindustries.com', hashedPassword, 'Admin', 'admin', now, now],
});
...
for (const cat of categories) {
  await db.execute({
    sql: 'INSERT OR IGNORE INTO Category (id, name, slug, description, "order", createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)',
    args: [cuid(), cat.name, cat.slug, cat.description, cat.order, now, now],
  });
}
console.log('Categories created');

const catRows = await db.execute('SELECT id, slug FROM Category');
const catMap = Object.fromEntries(catRows.rows.map(c => [c.slug, c.id]));
...
for (const p of products) {
  await db.execute({
    sql: 'INSERT OR IGNORE INTO Product (id, name, slug, description, features, categoryId, featured, active, "order", createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?)',
    args: [cuid(), p.name, p.slug, p.description, p.features, catMap[p.categorySlug], p.featured, p.order, now, now],
  });
}
